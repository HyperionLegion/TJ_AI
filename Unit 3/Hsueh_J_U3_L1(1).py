...
def max_value(state, turn, tc):
   ...
   for a, s in successors(state, turn):
      if turn=="X": 
         v = max(v, min_value(s, "O", tc)[0])
         if v ==min_value(s,"O", tc)[0]:
            newState = s
      else:
         v = max(v, min_value(s, "X", tc)[0])
         if v ==min_value(s,"X", tc)[0]:
            newState = s
   return v, newState
...
def min_value(state, turn, tc):
   ...
   for a, s in successors(state, turn):
      if turn=="X": 
         v = min(v, max_value(s,"O", tc)[0])
         if v ==max_value(s, "O", tc)[0]:
            newState = s
      else:
         v = min(v, max_value(s,"X", tc)[0])
         if v ==max_value(s, "X", tc)[0]:
            newState = s
   return v, newState
