...
def placeBlock(board, width, height, blocks, BLOCKCHAR, OPENCHAR, PROTECTEDCHAR):
   if blocks>0:
       ...
       board = recursive_backtracking(board, generate_possibles(board, width, height, OPENCHAR, BLOCKCHAR), blocks, width, height, BLOCKCHAR, OPENCHAR, PROTECTEDCHAR)
   return board
def heuristic(board, i, width, height, OPENCHAR, BLOCKCHAR):
    board = board[:i] + BLOCKCHAR + board[i+1:]
    board = board[:len(board)-i-1] + BLOCKCHAR + board[len(board)-i:]
    left = 0
    right = 0
    up = 0
    ...
    for j in range(0, width*height):
      if board[j] == BLOCKCHAR and j<=(width*height)/2:
         down+=1
      if board[j]==BLOCKCHAR and j >(width*height)/2:
         up+=1
      if board[j]==BLOCKCHAR and j%width <= width/2:
         left+=1
      if board[j] == BLOCKCHAR and j%width > width/2:
         right+=1
    return left*right + up*down
      
def generate_possibles(board, width, height, OPENCHAR, BLOCKCHAR):
   list = []
   for i in range(0,len(board)):
      if board[i] == OPENCHAR:
         list.append((i, heuristic(board, i, width, height, OPENCHAR, BLOCKCHAR)))
   list = sorted(list, key= lambda x: x[1])
   for i in range(0,len(list)):
      list[i] = list[i][0]
   return list
...
def recursive_backtracking(board, variables, blocks, width, height, BLOCKCHAR, OPENCHAR, PROTECTEDCHAR):
   ...
   for value in variables:
      if isValid(value,board, width, height, blocks, BLOCKCHAR, OPENCHAR, PROTECTEDCHAR):
         ...
         result = recursive_backtracking(board, generate_possibles(board, width, height, OPENCHAR, BLOCKCHAR), blocks, width, height, BLOCKCHAR, OPENCHAR, PROTECTEDCHAR)
